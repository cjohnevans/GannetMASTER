...
fit_method = FIT_LSQCURV; %FIT_NLINFIT;
...
%%%%%%%%%%%% GABA fit (Gaussian) %%%%%%%%%%%%%%%
  ...
  %residuals=resnorm;
  size(resnorm);
    % CJE Jan 2011 Initialise for GE data
  %GaussModelInit = [10*maxinGABA -9000 3.0 0 0 0.4]; %Initiate three gauss model parameters
  GaussModelInit = [10*maxinGABA -9000 2.9 0 0 0.4]; %Initiate three gauss model parameters
  ...
  lb = [0 -15000 2.7 -40*maxinGABA -2000*maxinGABA 0.1];
...
%  end
  ...
  Signal = GABAheight;
  Noise = std(real(MRS_struct.gabaspec((23660:25300))));
  Noise2 = std(real(MRS_struct.gabaspec((17150:17600))));
  MRS_struct.SNR = Signal/(2*Noise);
  %MRS_struct.SNR2 = Signal/Noise2
...
elseif strcmp(MRS_struct.Reference_compound,'Cr') 
  ...
  tmp = sprintf('GABA SNR     : %.4f', MRS_struct.SNR );%c
  text(0,-0.2, tmp, 'FontName', 'Courier');
